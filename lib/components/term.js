...
const { Menu } = remote
const shellMenu = require('../../app/menus/menus/shell')
...
export default class Term extends Component {
  ...
  componentDidMount() {
    ...
    this.getScreenNode().addEventListener('contextmenu', this.handleRightClick);
  ...
  handleRightClick(e) {
    ...
    // need a better way to get these
    const commands = {
    ...
    // Filter New Window option
    const shellMenuClean = shellMenu(commands, () => { /* noop */ })
      .submenu
      .filter(menu => {
        return menu.label !== 'New Window'
      })
    console.log(shellMenuClean)
    const RightClickMenu = Menu.buildFromTemplate(
      menuItems.concat(shellMenuClean)
    )
