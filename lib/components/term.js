/* global Blob,URL,requestAnimationFrame,config */
...
export default class Term extends Component {
  ...
  handleRightClick(e) {
    ...
    const enabled = !isCollapsed && selectedText.length > 1;

    const {quickEdit} = config.getConfig();
    const show = !quickEdit;
    ...
    const selectionMenu = [
      ...
      },
      {
        ...
        click() {
          openExternal(`https://www.google.com/search?q=${selectedText}`);
        },
        enabled
      }
    ];

    const menu = contextMenuTemplate.concat(selectionMenu);
